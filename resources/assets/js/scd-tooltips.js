/**
 * Tooltip functionality for Smart Cycle Discounts wizard
 * Matches the implementation from discounts-orchestrator.js
 * 
...
( function( $ ) {
	...
	$( document ).ready( function() {
		...
		$( document ).on( 'scd:content-updated', function() {
			initializeTooltips();
		} );
	...
	function initializeTooltips() {
		...
		$( document ).off( '.scd-tooltip' );
		
		// Target the actual help icon containers, not child elements
		$( document ).on( 'mouseenter.scd-tooltip', '.scd-field-helper', function( e ) {
			e.stopPropagation();
			var $trigger = $( this );
		...
		$( document ).on( 'mouseleave.scd-tooltip', '.scd-field-helper', function( e ) {
			e.stopPropagation();
			hideTooltip();
		} );

		// Keyboard accessibility - ensure help icons are focusable (only if not already set by PHP)
		$( '.scd-field-helper:not([tabindex])' ).attr( 'tabindex', '0' );

		$( document ).on( 'focus.scd-tooltip', '.scd-field-helper', function( e ) {
			var $trigger = $( this );
			var tooltipText = $trigger.attr( 'data-tooltip' );
		...
		$( document ).on( 'blur.scd-tooltip', '.scd-field-helper', function( e ) {
			hideTooltip();
		} );
	}
	...
	function showTooltip( $trigger, text ) {
		...
		if ( ! $tooltip || ! $tooltip.length ) {
			...
			$tooltip = $tooltipWrapper.find( '.scd-tooltip' ).first();

			// If still no tooltip found, create one
			if ( ! $tooltip.length ) {
				$tooltip = $( '<div class="scd-tooltip" role="tooltip" aria-hidden="true"></div>' );
				$tooltipWrapper.append( $tooltip );
			}
		}

		// Hide any other tooltips that might be visible (safety check)
		$tooltipWrapper.find( '.scd-tooltip' ).not( $tooltip ).removeClass( 'show' );
