...
( function( $, window, document ) {
	...
	TimelineVisualizer.defaults = {
		...
		colors: window.scdAdmin && window.scdAdmin.colors ? {
			background: window.scdAdmin.colors.background || '#f0f0f1',
			timeline: window.scdAdmin.colors.border || '#dcdcde',
			active: window.scdAdmin.colors.primary || '#0073aa',
			future: window.scdAdmin.colors.secondary || '#72aee6',
			past: window.scdAdmin.colors.text_muted || '#646970',
			current: window.scdAdmin.colors.danger || '#d63638',
			interactive: window.scdAdmin.colors.primary || '#2271b1',
			text: window.scdAdmin.colors.text || '#1e1e1e',
			grid: window.scdAdmin.colors.border_light || '#e0e0e0'
		} : {
			background: '#f0f0f1',
			timeline: '#dcdcde',
			active: '#0073aa',
	...
	TimelineVisualizer.prototype.createStructure = function() {
		this.container.empty().addClass( 'scd-timeline-visualizer' );
	...
	TimelineVisualizer.prototype.destroy = function() {
		...
		this.container.empty().removeClass( 'scd-timeline-visualizer' );
	};
	...
	window.SCD.TimelineVisualizer = TimelineVisualizer;

	// jQuery plugin
	$.fn.scdTimeline = function( options ) {
		return this.each( function() {
			...
			if ( !instance ) {
				...
				$this.data( 'scdTimeline', instance );
			}
