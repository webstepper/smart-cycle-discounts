 ...
 * @subpackage SmartCycleDiscounts/resources/assets/js/shared/ajax-service.js
...
( function( $ ) {
	...
	WSSCD.Shared.AjaxService = {
		...
		request: function( method, action, data, options ) {
			...
			var requestData = $.extend( {
				action: 'wsscd_ajax',          // Always use unified endpoint
				wsscdAction: action,           // Router converts camelCase to snake_case
				nonce: nonce
			}, data );
					...
					if ( 0 === jqXHR.status && 'error' === textStatus ) {
						...
						if ( window.console && window.console.info ) {
							console.info( '[AjaxService] Request cancelled (debouncing or navigation):', {
								action: data.wsscdAction || data.action,
		...
		getNonce: function( action ) {
			// Check analytics nonces first (for analytics_* and wsscd_analytics_* actions)
			if ( action && action.indexOf( 'analytics' ) !== -1 && window.wsscdAnalytics ) {
				if ( window.wsscdAnalytics.nonces && window.wsscdAnalytics.nonces.action_map ) {
					// Try action as-is first, then try with wsscd_ prefix (PHP uses full prefixed names)
					var analyticsNonceName = window.wsscdAnalytics.nonces.action_map[action] || window.wsscdAnalytics.nonces.action_map['wsscd_' + action];
					if ( analyticsNonceName && window.wsscdAnalytics.nonces[analyticsNonceName] ) {
						return window.wsscdAnalytics.nonces[analyticsNonceName];
					}
				}
				// Fallback to direct analytics nonce
				if ( window.wsscdAnalytics.nonce ) {
					return window.wsscdAnalytics.nonce;
				}
			}

			// Check wizard nonces
			var noncesObj = window.wsscdWizardData && window.wsscdWizardData.nonces;
			if ( noncesObj && noncesObj.action_map ) {
				// Try action as-is first, then try with wsscd_ prefix (PHP uses full prefixed names)
				var nonceName = noncesObj.action_map[action] || noncesObj.action_map['wsscd_' + action];
				if ( nonceName && noncesObj[nonceName] ) {
			...
			if ( window.wsscdWizardData ) {
				if ( window.wsscdWizardData.nonce ) {
					return window.wsscdWizardData.nonce;
				}
				if ( window.wsscdWizardData.wizardNonce ) {
					return window.wsscdWizardData.wizardNonce;
				}
			}

			// Try wsscdAdmin as another fallback
			if ( window.wsscdAdmin && window.wsscdAdmin.nonce ) {
				return window.wsscdAdmin.nonce;
			}
		...
		generateIdempotencyKey: function( action, data ) {
			...
			return 'wsscd_' + action + '_' + Math.abs( hash ) + '_' + timestamp + '_' + random;
		},
		...
		cancelPreviousSearchRequest: function( action, data ) {
			...
			var searchActions = [ 'wsscd_product_search', 'get_product_categories', 'search_categories' ];
		...
		setupGlobalHandlers: function() {
			...
			$( document ).ajaxComplete( function( event, jqXHR ) {
				if ( 401 === jqXHR.status ) {
					$( document ).trigger( 'wsscd:session:expired' );
				}
			} );
		},
		...
		upload: function( action, file, data, options ) {
			...
			formData.append( 'action', 'wsscd_ajax' );      // Always use unified endpoint
			formData.append( 'wsscdAction', action );        // Router converts camelCase to snake_case
	...
	WSSCD.Ajax = WSSCD.Shared.AjaxService;
