...
class SCD_API_Permissions {
    ...
    public function check_permissions(WP_REST_Request $request) {
        ...
        if (!$this->capability_manager->current_user_can($required_capability)) {
            $this->logger->warning('API access denied', array(
                'user_id' => $user_id,
                'route' => $route,
                'method' => $method,
        ...
        $this->logger->debug('API access granted', array(
            'user_id' => $user_id,
            'route' => $route,
            'method' => $method,
    ...
    public function add_capability_mapping(string $endpoint, string $method, string $capability): void {
        ...
        $this->logger->debug('API capability mapping added', array(
            'endpoint' => $endpoint,
            'method' => $method,
    ...
    public function add_public_endpoint(string $method, string $route): void {
        ...
        if (!in_array($endpoint_key, $this->public_endpoints)) {
            ...
            $this->logger->debug('Public endpoint added', array(
                'method' => $method,
                'route' => $route
            ));
        }
