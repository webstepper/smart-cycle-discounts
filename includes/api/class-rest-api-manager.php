...
class SCD_REST_API_Manager {
    ...
    public function register_routes(): void {
        ...
        $this->logger->debug('REST API routes registered', array(
    ...
    public function register_fields(): void {
        ...
        register_rest_field('product', 'scd_discount_info', array(
    ...
    public function register_endpoint(string $endpoint_class): bool {
        ...
        try {
            ...
            $this->logger->error('Failed to register endpoint', array(
    ...
    private function get_rate_limit_for_request(WP_REST_Request $request): array {
        ...
        if (strpos($route, '/analytics') !== false) {
            return array('limit' => 50, 'window' => 3600);
        }
        
        // Campaign creation
        if (strpos($route, '/campaigns') !== false && $method === 'POST') {
            return array('limit' => 20, 'window' => 3600);
        }
        
        // Discount calculation endpoints - more restrictive
        if (strpos($route, '/discounts/calculate/bulk') !== false) {
            return array('limit' => 10, 'window' => 300); // 10 requests per 5 minutes
        }
        
        if (strpos($route, '/discounts/calculate') !== false) {
            return array('limit' => 30, 'window' => 300); // 30 requests per 5 minutes
        }
        
        if (strpos($route, '/discounts/preview') !== false) {
            return array('limit' => 60, 'window' => 300); // 60 requests per 5 minutes
        }
        ...
        if (strpos($route, '/discounts/best') !== false && $method === 'GET') {
            return array('limit' => 100, 'window' => 3600); // 100 per hour
        }
    ...
    private function get_allowed_origin(WP_REST_Request $request): string {
        ...
        $allowed_origins = apply_filters('scd_api_allowed_origins', array(
            home_url(),
            admin_url()
        ));
    ...
    private function get_rate_limits(): array {
        return array(
            'default' => array('limit' => 100, 'window' => 3600),
            'analytics' => array('limit' => 50, 'window' => 3600),
            'campaigns_create' => array('limit' => 20, 'window' => 3600)
        );
    }
