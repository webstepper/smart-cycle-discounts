...
class SCD_Discounts_Controller {
    ...
    public function register_routes(): void {
        ...
        register_rest_route($this->namespace, '/discounts', array(
        ...
        register_rest_route($this->namespace, '/discounts/calculate', array(
        ...
        register_rest_route($this->namespace, '/discounts/calculate/bulk', array(
            'methods' => WP_REST_Server::CREATABLE,
        ...
        // Discount preview
        register_rest_route($this->namespace, '/discounts/preview', array(
            'methods' => WP_REST_Server::CREATABLE,
        ...
        register_rest_route($this->namespace, '/discounts/campaign/(?P<campaign_id>\d+)', array(
            'methods' => WP_REST_Server::READABLE,
        ...
        register_rest_route($this->namespace, '/discounts/best/(?P<product_id>\d+)', array(
            'methods' => WP_REST_Server::READABLE,
        ...
        // Discount validation
        register_rest_route($this->namespace, '/discounts/validate', array(
            'methods' => WP_REST_Server::CREATABLE,
            ...
            'args' => array(
                ...
                'bogo_config' => array(
                    ...
                    'type' => 'object',
                    'properties' => array(
                        'buy_quantity' => array('type' => 'integer', 'minimum' => 1),
                        'get_quantity' => array('type' => 'integer', 'minimum' => 1),
                        'discount_percentage' => array('type' => 'number', 'minimum' => 0, 'maximum' => 100)
                    )
                ),
                'bundle_config' => array(
                    ...
                    'type' => 'object',
                    'properties' => array(
                        'bundle_size' => array('type' => 'integer', 'minimum' => 2),
                        'bundle_price' => array('type' => 'number', 'minimum' => 0.01)
                    )
                )
            )
        ));
    ...
    public function get_active_discounts(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Active discounts retrieved via API', array(
                'count' => count($data),
                'params' => $params
            ));
            ...
            $this->logger->error('Failed to get active discounts via API', array(
                'error' => $e->getMessage(),
    ...
    public function calculate_discount(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Discount calculated via API', array(
                'product_id' => $product_id,
                'campaign_id' => $campaign_id,
            ...
            $this->logger->error('Failed to calculate discount via API', array(
                'error' => $e->getMessage(),
    ...
    public function calculate_bulk_discounts(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Bulk discounts calculated via API', array(
            ...
            $this->logger->error('Failed to calculate bulk discounts via API', array(
                'error' => $e->getMessage(),
                'params' => $request->get_params()
            ));
    ...
    public function preview_discount(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Discount preview calculated via API', array(
                'type' => $discount_type,
                'value' => $discount_value,
            ...
            $this->logger->error('Failed to preview discount via API', array(
                'error' => $e->getMessage(),
                'params' => $request->get_params()
            ));
    ...
    public function get_campaign_discounts(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $data = array(
                ...
                'active_discounts' => array_map(function($discount) use ($request) {
                    return $this->prepare_discount_for_response($discount, $request);
                }, $discounts),
                'total_discounts' => count($discounts)
            );

            $this->logger->debug('Campaign discounts retrieved via API', array(
                'campaign_id' => $campaign_id,
            ...
            $this->logger->error('Failed to get campaign discounts via API', array(
                'error' => $e->getMessage(),
    ...
    public function get_best_discount(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Best discount retrieved via API', array(
                'product_id' => $product_id,
                'has_discount' => $best_discount->has_discount()
            ));
            ...
            $this->logger->error('Failed to get best discount via API', array(
                'error' => $e->getMessage(),
                'product_id' => $product_id ?? null
            ));
    ...
    public function validate_discount(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Discount validation performed via API', array(
                'type' => $discount_type,
                'value' => $discount_value,
            ...
            $this->logger->error('Failed to validate discount via API', array(
                'error' => $e->getMessage(),
                'params' => $request->get_params()
            ));
