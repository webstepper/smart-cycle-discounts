 ...
 * @subpackage SmartCycleDiscounts/includes/api/endpoints/class-campaigns-controller.php
...
class SCD_Campaigns_Controller {
	...
	public function get_campaigns( WP_REST_Request $request ): WP_REST_Response {
		try {
			...
			$this->logger->error(
				'Failed to get campaigns via API',
				array(
					'error'  => $e->getMessage(),
	...
	public function get_campaign( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			$this->logger->error(
				'Failed to get campaign via API',
				array(
	...
	public function create_campaign( WP_REST_Request $request ): WP_REST_Response {
		try {
			...
			$this->logger->error(
				'Failed to create campaign via API',
				array(
					'error' => $e->getMessage(),
	...
	public function update_campaign( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			$this->logger->error(
				'Failed to update campaign via API',
				array(
					'campaign_id' => $campaign_id,
					'error'       => $e->getMessage(),
				)
	...
	public function delete_campaign( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			$this->logger->error(
				'Failed to delete campaign via API',
				array(
					'campaign_id' => $campaign_id,
					'error'       => $e->getMessage(),
				)
	...
	public function campaign_action( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			$this->logger->info(
				'Campaign action performed via API',
				array(
					'campaign_id' => $campaign_id,
					'action'      => $action,
				)
			...
			$this->logger->error(
				'Failed to perform campaign action via API',
				array(
					'campaign_id' => $campaign_id,
					'action'      => $action,
					'error'       => $e->getMessage(),
	...
	public function batch_campaigns( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			$this->logger->info(
				'Bulk campaign action performed via API',
				array(
					'action'    => $action,
			...
			$this->logger->error(
				'Failed to perform bulk campaign action via API',
				array(
					'action' => $action,
					'ids'    => $campaign_ids,
					'error'  => $e->getMessage(),
