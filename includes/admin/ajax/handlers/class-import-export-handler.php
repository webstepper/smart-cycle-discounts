...
class SCD_Import_Export_Handler extends SCD_Abstract_Ajax_Handler {
	...
	protected function handle( $request ) {
		...
		// Log request start
		$this->logger->flow( 'info', 'AJAX START', 'Processing export request', array(
		...
		if ( ! empty( $export_type ) ) {
			...
			$this->logger->flow( 'error', 'AJAX ERROR', 'Invalid export request', array(
	...
	private function handle_export( $type, $start_time ) {
		...
		if ( $this->feature_gate && ! $this->feature_gate->can_export_data() ) {
			$this->logger->flow( 'warning', 'AJAX BLOCKED', 'Export blocked - premium feature', array(
				'export_type' => $type,
				'user_id' => get_current_user_id(),
				'_start_time' => $start_time
			) );
		...
		if ( 'campaigns' === $type ) {
			...
			$this->logger->flow( 'error', 'AJAX ERROR', 'Invalid export type', array(
				'export_type' => $type,
				'_start_time' => $start_time
			) );
	...
	private function export_campaigns( $start_time ) {
		...
		if ( null === $campaigns && ! empty( $wpdb->last_error ) ) {
			$this->logger->flow( 'error', 'DB ERROR', 'Failed to export campaigns', array(
		...
		$this->logger->flow( 'notice', 'AJAX SUCCESS', 'Campaigns exported successfully', array(
			'count' => count( $sanitized_campaigns ),
			'ownership_filtered' => $is_filtered,
			'filename' => $filename,
			'size' => strlen( $content ),
	...
	private function export_settings( $start_time ) {
		...
		// Log successful export
		$this->logger->flow( 'notice', 'AJAX SUCCESS', 'Settings exported successfully', array(
