					...
					if ( $min_price > 0 && $max_price > 0 ) {
						...
						if ( $price_variance >= 5.0 && $discount_value > 0 ) {
							...
							$warnings[] = array(
								...
								'message' => sprintf(
									__( 'Your %s fixed discount will have dramatically different effects across your price range (%s - %s). Cheap products get %d%% off while expensive products only get %d%% off.', 'smart-cycle-discounts' ),
									$this->_format_price_plain( $discount_value ),
			...
			if ( $discount_value >= 90 && $discount_value < 100 ) {
				...
				if ( ! empty( $product_prices ) ) {
					...
					$warnings[] = array(
						...
						'message' => sprintf(
							__( 'A %d%% discount is extremely high and will significantly impact profit margins. For example, a %s product becomes %s. Verify this discount level is intentional and sustainable.', 'smart-cycle-discounts' ),
							$discount_value,
				...
				if ( ! empty( $low_stock_products ) ) {
					...
					foreach ( $sample_products as $prod ) {
						$product_list[] = sprintf(
							__( '%s (Stock: %d, Required: %d)', 'smart-cycle-discounts' ),
							$prod['name'],
							$prod['stock'],
							$prod['required']
						);
					...
					$message = sprintf(
						__( 'Your "Buy %d Get %d" campaign includes %d product(s) with insufficient stock. Customers will not be able to complete the offer for these items.', 'smart-cycle-discounts' ),
						$buy_quantity,
						$get_quantity,
						$count
					);
		...
		if ( 'tiered' === $discount_type ) {
			...
			if ( 'quantity' === $tier_type && ! empty( $tiers ) ) {
				...
				if ( ! empty( $product_stock ) ) {
					...
					$valid_stock = array_filter( $product_stock, function( $stock ) {
						return null !== $stock;
					} );
					...
					if ( $max_stock > 0 ) {
						...
						foreach ( $tiers as $tier ) {
							...
							if ( $threshold > $max_stock ) {
								...
								$unreachable_tiers[] = array(
									'threshold' => $threshold,
									'description' => sprintf(
										__( 'Buy %d+ get %s', 'smart-cycle-discounts' ),
										$threshold,
										$tier_description
									)
								);
						...
						if ( ! empty( $unreachable_tiers ) ) {
							...
							$warnings[] = array(
								...
								'message' => sprintf(
									__( '%d discount tier(s) cannot be reached because the highest stock level is %d units. For example, the tier "%s" requires %d items but maximum available stock is only %d.', 'smart-cycle-discounts' ),
									$count,
									$max_stock,
