...
class WSSCD_Discount_Display_Rules {
	...
	public function can_display_on_shop( string $discount_type ): bool {
		...
		$settings = get_option( 'wsscd_badge_settings', array() );
	 ...
	 */
	public function get_simple_badge_text( WSSCD_Campaign $campaign ): string {
			...
			case 'tiered':
				...
				if ( ! empty( $tiers ) ) {
					...
					if ( 'percentage' === $tier_mode ) {
						/* translators: %d: minimum discount percentage */
					return sprintf( __( 'From %d%% OFF', 'smart-cycle-discounts' ), absint( $min_discount ) );
					} else {
						/* translators: %s: formatted minimum discount amount */
