 ...
 * @subpackage SmartCycleDiscounts/includes/cli/class-wsscd-cli-health-check.php
...
class WSSCD_CLI_Health_Check {
	 ...
	 *     $ wp wsscd health-check
	 *
	 *     # Run with detailed report
	 *     $ wp wsscd health-check --report
	 *
	 *     # Output as JSON
	 *     $ wp wsscd health-check --format=json
	 *
	 *     # Attempt to fix issues
	 *     $ wp wsscd health-check --fix
	 *
	 * @when after_wp_load
	 */
	public function __invoke( $args, $assoc_args ) {
		...
		try {
			...
			if ( $registry ) {
				$health_check = new WSSCD_Service_Health_Check( $container, $registry );
	...
	private function output_csv( $results ) {
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- Writing to php://output for CLI CSV output.
		...
		foreach ( $results['checks'] as $check ) {
			fputcsv(
				...
				array(
					...
					$check['name'],
					$check['status'],
					wp_json_encode( $check['details'] ?? array() ),
				)
			);
		}
	...
	private function attempt_fixes( $results, $container ) {
		...
		if ( ! empty( $missing_services ) ) {
			...
			// Services are registered via WSSCD_Service_Definitions
			// If services are missing, this indicates a configuration error
			WP_CLI::warning( 'Service registration system may have errors. Check WSSCD_Service_Definitions.' );
		}
...
WP_CLI::add_command( 'wsscd health-check', 'WSSCD_CLI_Health_Check' );
