 ...
 *
 * Provides CLI commands for service health checks
 *
...
class SCD_CLI_Health_Check {
    ...
    private function output_table($results) {
        ...
        if (!empty($results['checks'])) {
            ...
            WP_CLI\Utils\format_items('table', $table_data, ['Check', 'Status', 'Details']);
        }
    ...
    private function output_csv($results) {
        ...
        // Headers
        fputcsv($output, ['Type', 'Name', 'Status', 'Details']);
        
        // Summary
        fputcsv($output, ['Summary', 'Overall Status', $results['status'], '']);
        ...
        // Errors
        foreach ($results['errors'] as $error) {
            fputcsv($output, ['Error', '', 'error', $error]);
        }
        
        // Warnings
        foreach ($results['warnings'] as $warning) {
            fputcsv($output, ['Warning', '', 'warning', $warning]);
        }
