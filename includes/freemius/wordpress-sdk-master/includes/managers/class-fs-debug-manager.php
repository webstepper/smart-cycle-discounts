...
class FS_DebugManager {
	...
	static function _debug_page_actions() {
		...
		} elseif ( fs_request_is_action( 'reset_deactivation_snoozing' ) ) {
	...
	private static function get_all_licenses_by_module_type() {
		...
		foreach ( $licenses as $module_id => $module_licenses ) {
			...
			$licenses_by_module_type[ $fs->get_module_type() ] = array_merge(
				$licenses_by_module_type[ $fs->get_module_type() ],
				$module_licenses
			);
		}
