 ...
 *
 * Centralized time handling functions using DateTimeImmutable and wp_timezone()
 * Ensures correct handling of timezones and DST transitions
 *
...
function scd_get_canonical_timezone(): string {
    ...
    if ( preg_match( '/^UTC[+-]/', $wp_tz ) ) {
        ...
        if ( function_exists( 'scd_log_warning' ) ) {
            scd_log_warning( 'WordPress timezone is set to UTC offset instead of timezone string', array(
                'timezone' => $wp_tz,
    ...
    if ( 'spring_forward' === $dst_info['transition_type'] && $dst_info['lost_hour'] ) {
        ...
        if ( $time_parts && $start_parts && $end_parts ) {
            ...
            if ( $time_minutes >= $start_minutes && $time_minutes < $end_minutes ) {
                ...
                // Log the adjustment
                if ( function_exists( 'scd_log_warning' ) ) {
                    scd_log_warning( 'DST Spring Forward: Adjusted time from non-existent hour', array(
        ...
        if ( $time_parts && $start_parts && $end_parts ) {
            ...
            if ( $time_minutes >= $start_minutes && $time_minutes < $end_minutes ) {
                ...
                if ( function_exists( 'scd_log_info' ) ) {
                    scd_log_info( 'DST Fall Back: Time in repeated hour, using first occurrence', array(
                        'time' => $time_str,
                        'date' => $datetime->format( 'Y-m-d' )
                    ) );
                }
