 ...
 * @subpackage SmartCycleDiscounts/includes/utilities/wsscd-debug-functions.php
...
if ( ! function_exists( 'wsscd_debug' ) ) {
	/**
	 * Get the debug logger instance.
	 *
	 * @since    1.0.0
	 * @return   WSSCD_Debug_Logger|null    Debug logger instance or null if not available.
	 */
	function wsscd_debug() {
		...
		if ( null === $debug_logger ) {
			...
			if ( $container && method_exists( $container, 'get' ) && method_exists( $container, 'has' ) && $container->has( 'debug_logger' ) ) {
				try {
					...
					if ( ! class_exists( 'WSSCD_Debug_Logger' ) ) {
						require_once WSSCD_INCLUDES_DIR . 'utilities/class-debug-logger.php';
					}
					$debug_logger = new WSSCD_Debug_Logger();
				}
			} else {
...
if ( ! function_exists( 'wsscd_debug_wizard' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_wizard( string $step, array $init_data = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_navigation' ) ) {
	/**
	 ...
	 * @return   void
	 */
	function wsscd_debug_navigation( string $from_step, string $to_step, string $action, array $context = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_ajax' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_ajax( string $action, array $params = array(), string $nonce = '' ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_ajax_response' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_ajax_response( string $action, $response, bool $success, float $duration ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_validation' ) ) {
	/**
	 ...
	 * @return   void
	 */
	function wsscd_debug_validation( string $step, array $data, array $rules, bool $valid, array $errors = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_sanitization' ) ) {
	/**
	 ...
	 * @return   void
	 */
	function wsscd_debug_sanitization( string $field, $raw_value, $clean_value, string $method ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_database' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_database( string $operation, string $table, array $data, $result, float $duration ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_persistence' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_persistence( string $action, string $step, array $data, bool $success, string $message = '' ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_interaction' ) ) {
	/**
	 ...
	 * @return   void
	 */
	function wsscd_debug_interaction( string $interaction, string $element, $value = null, array $context = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_error' ) ) {
	 ...
	 * @return   void
	 */
	function wsscd_debug_error( string $message, \Throwable $exception, array $context = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_performance' ) ) {
	/**
	 ...
	 * @return   void
	 */
	function wsscd_debug_performance( string $operation, float $duration, array $metrics = array() ): void {
		$logger = wsscd_debug();
		if ( $logger ) {
...
if ( ! function_exists( 'wsscd_debug_end' ) ) {
	/**
	 * Log request end.
	 *
	 * @since    1.0.0
...
if ( WSSCD_DEBUG ) {
	register_shutdown_function( 'wsscd_debug_end' );
}
