 ...
 * @subpackage SmartCycleDiscounts/includes/utilities/wsscd-debug-console-functions.php
 ...
 */
function wsscd_is_debug_enabled(): bool {
 ...
 *
 * @since    1.0.0
 * @return   WSSCD_Debug_Console|null    Debug console instance or null.
 */
function wsscd_get_debug_console() {
	if ( ! wsscd_is_debug_enabled() ) {
		return null;
	}
 ...
 * @return   void
 */
function wsscd_debug( string $message, array $context = array() ): void {
	$console = wsscd_get_debug_console();
	if ( $console ) {
 ...
 * @return   void
 */
function wsscd_info( string $message, array $context = array() ): void {
	$console = wsscd_get_debug_console();
	if ( $console ) {
 ...
 * @return   void
 */
function wsscd_warning( string $message, array $context = array() ): void {
	$console = wsscd_get_debug_console();
	if ( $console ) {
 ...
 * @return   void
 */
function wsscd_error( string $message, array $context = array() ): void {
	$console = wsscd_get_debug_console();
	if ( $console ) {
 ...
 */
function wsscd_get_debug_info( int $campaign_id = 0 ): array {
	if ( ! wsscd_is_debug_enabled() ) {
	...
	try {
		...
		return array(
			...
			'system_info'            => array(
				...
				'debug_mode'   => wsscd_is_debug_enabled(),
			),
		);
 ...
 */
function wsscd_get_campaign( int $campaign_id ) {
	if ( ! wsscd_is_debug_enabled() ) {
 ...
 */
function wsscd_test_validation( string $step, array $data ) {
	if ( ! wsscd_is_debug_enabled() ) {
		return 'Debug mode not enabled';
	}
	...
	try {
		...
		$result  = WSSCD_Validation::validate( $data, $context );
 ...
 */
function wsscd_run_validation_tests( string $step = 'basic' ) {
	if ( ! wsscd_is_debug_enabled() ) {
		return 'Debug mode not enabled';
	}
	...
	switch ( $step ) {
		case 'basic':
			$test_scenarios = array(
				...
				'invalid_chars'    => array(
					'name'        => 'Campaign<invalid>special&chars',
	...
	foreach ( $test_scenarios as $test_name => $test_data ) {
		$test_result = wsscd_test_validation( $step, $test_data );
 ...
 */
function wsscd_get_validation_rules() {
	if ( ! wsscd_is_debug_enabled() ) {
		return 'Debug mode not enabled';
	}

	try {
		$rules = WSSCD_Validation::get_js_data();
