...
require_once dirname(__FILE__) . '/class-session-lock-service.php';
...
class SCD_Session_Service {
    ...
    private function save_internal(): bool|WP_Error {
        ...
        if ($data_size > $this->max_data_size) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                SCD_Log::warning('Session: Data too large', array(
                    'data_size' => $data_size, 'max_size' => $this->max_data_size,
                    'session_id' => $this->session_id
                ));
            }
            return new WP_Error(
                ...
                sprintf(
                    __('The data you are trying to save is too large (%s). Maximum allowed size is %s. Please reduce the amount of data or contact support.', 'smart-cycle-discounts'),
    ...
    public function set(string $key, mixed $value): bool|WP_Error {
        ...
        try {
            ...
            $sanitized = $this->sanitize_data([$key => $value]);
            ...
            if ($data_size > $this->max_data_size) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    SCD_Log::warning('Session: Key too large', array(
                        'key' => $key,
                        'max_size' => $this->max_data_size,
                ...
                return new WP_Error(
                    'session_data_too_large',
                    sprintf(
                        __('The data for "%s" is too large (%s). Maximum total size is %s. Please reduce the amount of data.', 'smart-cycle-discounts'),
                        $key,
                        size_format($data_size),
                        size_format($this->max_data_size)
                    ),
    ...
    public function set_data(array $data): bool|WP_Error {
        ...
        try {
            ...
            if ($data_size > $this->max_data_size) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    SCD_Log::warning('Session: Data too large for set_data', array(
                        'max_size' => $this->max_data_size, 'projected_size' => $data_size
                    ));
                }
                return new WP_Error(
                    'session_data_too_large',
                    sprintf(
                        __('The total data size (%s) exceeds the maximum allowed (%s). Please reduce the amount of data being saved.', 'smart-cycle-discounts'),
                        size_format($data_size),
                        size_format($this->max_data_size)
                    ),
                    array(
    ...
    private function validate_session_ownership(array $data): bool {
        ...
        if ($validate_ip && isset($data['ip_address']) && !empty($data['ip_address'])) {
            ...
            if ($current_ip !== $data['ip_address']) {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    SCD_Log::warning('Session: IP mismatch detected', array(
                        'session_ip' => $data['ip_address'], 'current_ip' => $current_ip,
