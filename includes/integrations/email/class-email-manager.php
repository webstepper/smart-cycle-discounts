...
class SCD_Email_Manager {
    ...
    private function init_provider(): void {
        ...
        $this->logger->debug( 'Email provider initialized', array(
    ...
    public function send_campaign_started_notification(int $campaign_id): void {
        ...
        try {
            ...
            $this->logger->info('Campaign started notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name()
            ));

        } catch (Exception $e) {
            $this->logger->error('Failed to send campaign started notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function send_campaign_ending_notification(int $campaign_id): void {
        ...
        try {
            ...
            $this->logger->info('Campaign ending notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name(),
                'time_remaining' => $time_remaining
            ));
            ...
            $this->logger->error('Failed to send campaign ending notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function send_campaign_ended_notification(int $campaign_id): void {
        ...
        try {
            ...
            $this->logger->info('Campaign ended notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name(),
            ...
            $this->logger->error('Failed to send campaign ended notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function send_daily_performance_report(string $date): void {
        ...
        try {
            ...
            $this->logger->info('Daily performance report queued', array(
                'date' => $date, 'revenue' => $report_data['total_revenue']
            ));

        } catch (Exception $e) {
            $this->logger->error('Failed to send daily performance report', array(
                'date' => $date,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function send_weekly_performance_report(string $week_start): void {
        ...
        try {
            ...
            $this->logger->info('Weekly performance report queued', array(
            ...
            $this->logger->error('Failed to send weekly performance report', array(
                'week_start' => $week_start,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function send_performance_alert( int $campaign_id, array $alert_data ): void {
        ...
        try {
            ...
            $this->logger->info( 'Performance alert notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name(),
            ...
            $this->logger->error( 'Failed to send performance alert notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ) );
        }
    ...
    public function send_low_stock_alert( int $campaign_id, array $stock_data ): void {
        ...
        try {
            ...
            $this->logger->info( 'Low stock alert notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name(),
                'total_low_stock' => $total_low_stock
            ) );
            ...
            $this->logger->error( 'Failed to send low stock alert notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ) );
        }
    ...
    public function send_milestone_alert( int $campaign_id, array $milestone_data ): void {
        ...
        try {
            ...
            $this->logger->info( 'Milestone alert notification queued', array(
                'campaign_id' => $campaign_id,
                'campaign_name' => $campaign->get_name(),
            ...
            $this->logger->error( 'Failed to send milestone alert notification', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ) );
        }
    ...
    public function queue_email( string $template, array $variables, array $recipients = array() ): bool {
        ...
        $this->logger->debug( 'Email queued', array(
            'template' => $template,
    ...
    public function process_email_queue(): void {
        ...
        $this->email_queue = array_filter($this->email_queue, function($email) {
    ...
    private function send_email( array $email_data ): bool {
        try {
            ...
            $this->logger->error( 'Failed to send email', array(
                'template' => $email_data['template'],
                'error' => $e->getMessage(),
            ) );
    ...
    private function render_basic_email_template(string $template, array $variables): string {
        $content = '<html><body>';
        $content .= '<h2>' . esc_html(ucwords(str_replace(['_', '-'], ' ', $template))) . '</h2>';
    ...
    private function is_notification_enabled( string $notification ): bool {
        ...
        if ( ! $this->feature_gate->can_send_notification( $notification ) ) {
            $this->logger->debug( 'Notification blocked by licensing', array(
    ...
    private function format_performance_summary(array $performance): string {
        return sprintf(
            __('Revenue: %s | Orders: %d | Conversion Rate: %.2f%%', 'smart-cycle-discounts'),
