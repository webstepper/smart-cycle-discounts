...
class SCD_Customer_Usage_Manager {
        ...
        try {
            ...
            if (!$can_use) {
                ...
                return array(
                    'valid' => false,
                    'error' => sprintf(
                        __('You have already used this discount %d time(s). Maximum allowed: %d', 'smart-cycle-discounts'),
                        $usage['usage_count'] ?? 0,
                        $max_uses
                    ),
            ...
            $this->logger->error('Failed to validate customer usage', array(
        ...
        try {
            ...
            foreach ($order->get_items() as $item) {
                ...
                if ($original_price <= 0 || $item_total < 0) {
                    $this->logger->warning('Invalid price data for usage tracking', array(
                        'order_id' => $order_id,
                        'item_id' => $item->get_id(),
                ...
                if ($result) {
                    $this->logger->info('Customer usage recorded', array(
                        'order_id' => $order_id,
                        'campaign_id' => $campaign_id,
                        'customer_email' => $customer_email
                    ));
    ...
    private function get_customer_email(): ?string {
        try {
            ...
            $this->logger->error('Failed to get customer email', array(
                'error' => $e->getMessage()
            ));
        }
    ...
    public function get_customer_statistics(string $customer_email): array {
        try {
            ...
            $this->logger->error('Failed to get customer statistics', array(
                'customer_email' => $customer_email,
                'error' => $e->getMessage()
            ));
