...
class SCD_Campaign_Serializer {
    ...
    public function serialize(SCD_Campaign $campaign, array $context = array()): array {
        try {
            ...
            $this->logger->error('Campaign serialization failed', array(
    ...
    public function deserialize(array $data): array {
        try {
            ...
            if (isset($data['category_ids']) && is_array($data['category_ids'])) {
                ...
                $validated['category_ids'] = array_map(function($id) {
                    if ('all' === $id) {
                        return 'all';
                    }
            ...
            $this->logger->error('Campaign deserialization failed', array(
                'data' => $data,
                'error' => $e->getMessage()
            ));
    ...
    private function parse_datetime(string $datetime): ?DateTime {
        try {
            ...
            $this->logger->warning('Invalid datetime format', array(
                'datetime' => $datetime,
                'error' => $e->getMessage()
            ));
            return null;
