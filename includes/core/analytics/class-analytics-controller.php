...
class SCD_Analytics_Controller {
    ...
    public function register_routes(): void {
        ...
        // Campaign analytics
        register_rest_route($this->namespace, '/analytics/campaigns', array(
            'methods' => WP_REST_Server::READABLE,
            'callback' => array($this, 'get_campaigns_analytics'),
            'permission_callback' => array($this->permissions_manager, 'check_analytics_permissions'),
            'args' => array_merge($this->get_date_range_params(), array(
                'campaign_id' => array(
        ...
        // Product analytics
        register_rest_route($this->namespace, '/analytics/products', array(
            'methods' => WP_REST_Server::READABLE,
        ...
        // Chart data
        register_rest_route($this->namespace, '/analytics/charts/(?P<type>revenue|conversions|clicks|views)', array(
            'methods' => WP_REST_Server::READABLE,
        ...
        // Top performers
        register_rest_route($this->namespace, '/analytics/top/(?P<entity>campaigns|products)', array(
            'methods' => WP_REST_Server::READABLE,
    ...
    public function get_overview(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Overview analytics retrieved via API', array(
            ...
            $this->logger->error('Failed to get overview analytics via API', array(
                'error' => $e->getMessage(),
    ...
    public function get_campaigns_analytics(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Campaigns analytics retrieved via API', array(
                'campaign_id' => $campaign_id,
                'date_range' => $date_range,
            ...
            $this->logger->error('Failed to get campaigns analytics via API', array(
                'error' => $e->getMessage(),
    ...
    public function get_products_analytics(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Products analytics retrieved via API', array(
                'product_id' => $product_id,
                'date_range' => $date_range,
            ...
            $this->logger->error('Failed to get products analytics via API', array(
                'error' => $e->getMessage(),
    ...
    public function get_realtime_metrics(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->error('Failed to get real-time metrics via API', array(
                'error' => $e->getMessage()
            ));
    ...
    public function get_chart_data(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Chart data retrieved via API', array(
                'type' => $type,
                'granularity' => $granularity,
            ...
            $this->logger->error('Failed to get chart data via API', array(
                'error' => $e->getMessage(),
                'type' => $type ?? 'unknown',
                'params' => $request->get_params()
            ));
    ...
    public function get_top_performers(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->debug('Top performers retrieved via API', array(
                'entity' => $entity,
                'metric' => $metric,
            ...
            $this->logger->error('Failed to get top performers via API', array(
                'error' => $e->getMessage(),
    ...
    public function export_analytics(WP_REST_Request $request): WP_REST_Response {
        try {
            ...
            $this->logger->info('Analytics data exported via API', array(
            ...
            $this->logger->error('Failed to export analytics via API', array(
                'error' => $e->getMessage(),
                'params' => $request->get_params()
            ));
