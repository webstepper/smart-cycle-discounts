...
class SCD_Analytics_Collector {
    ...
    public function track_event(
        ...
        if (!in_array($event_type, $this->event_types)) {
            $this->logger->warning('Invalid event type for analytics tracking', array(
                'event_type' => $event_type, 'valid_types' => $this->event_types
            ));
            return false;
        }
        ...
        try {
            ...
            if ($result) {
                $this->logger->debug('Analytics event tracked', array(
            ...
            $this->logger->error('Failed to track analytics event', array(
                'event_type' => $event_type,
                'error' => $e->getMessage(),
                'event_data' => $event_data
            ));
    ...
    public function track_discount_view(int $campaign_id, int $product_id, array $discount_data = array()): bool {
        $event_data = array_merge($discount_data, array(
    ...
    public function get_campaign_analytics(
        ...
        try {
            ...
            $this->logger->error('Failed to get campaign analytics', array(
                'campaign_id' => $campaign_id,
                'error' => $e->getMessage()
            ));
    ...
    private function update_campaign_conversion_metrics(int $campaign_id, $order, $item): void {
        ...
        if ( false === $result ) {
            $this->logger->error( 'Failed to update campaign conversion metrics', array(
                'campaign_id' => $campaign_id,
            ...
            $this->logger->debug( 'Campaign conversion metrics updated', array(
                'campaign_id' => $campaign_id,
                'order_id' => $order->get_id(),
    ...
    public function get_campaign_performance( array $args = array(), int $limit = 10, string $sort_by = 'revenue', string $sort_order = 'desc' ): array {
        ...
        try {
            ...
            $this->logger->error( 'Failed to get campaign performance', array(
                'error' => $e->getMessage(),
    ...
    public function get_revenue_trend( string $date_range = '30days', string $granularity = 'daily', int $campaign_id = 0 ): array {
        ...
        try {
            ...
            $this->logger->error( 'Failed to get revenue trend', array(
                'error' => $e->getMessage(),
                'date_range' => $date_range,
                'granularity' => $granularity,
                'campaign_id' => $campaign_id
    ...
    private function aggregate_hourly_metrics(): void {
        ...
        if ( $result === false ) {
            $this->logger->error( 'Hourly aggregation query failed', array(
                'error' => $wpdb->last_error,
                'hour' => $previous_hour
            ) );
        } else {
            $this->logger->debug( 'Hourly aggregation completed', array(
                'hour' => $previous_hour,
    ...
    private function aggregate_daily_metrics(): void {
        ...
        if ( $result === false ) {
            $this->logger->error( 'Daily aggregation query failed', array(
                'error' => $wpdb->last_error,
                'date' => $yesterday
            ) );
        } else {
            $this->logger->debug( 'Daily aggregation completed', array(
                'date' => $yesterday,
                'rows_affected' => $result
            ) );
    ...
    private function cleanup_old_hourly_data(): void {
        ...
        if ( $deleted ) {
            $this->logger->debug( 'Cleaned up old hourly data', array(
    ...
    public function run_hourly_aggregation(): void {
        try {
            ...
            $this->logger->error('Hourly analytics aggregation failed', array(
                'error' => $e->getMessage()
            ));
        }
    }
    ...
    public function run_daily_aggregation(): void {
        try {
            ...
            $this->logger->error('Daily analytics aggregation failed', array(
                'error' => $e->getMessage()
            ));
        }
    }
    ...
    public function get_top_products_by_revenue( string $date_range = '30days', int $limit = 10 ): array {
        ...
        try {
            ...
            $this->logger->error( 'Failed to get top products by revenue', array(
                'error' => $e->getMessage(),
                'date_range' => $date_range
            ) );
