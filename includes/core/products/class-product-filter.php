...
class SCD_Product_Filter {
    ...
    public function apply_filters(array $product_ids, array $filters): array {
        ...
        $this->logger->debug('Filters applied to products', array(
    ...
    public function filter_by_category(array $product_ids, array $config): array {
        ...
        if ( count( $categories ) > 100 ) {
            $this->logger->warning( 'Too many categories provided, limiting to 100', array(
        ...
        return array_filter($product_ids, function($product_id) use ($category_ids, $operator) {
    ...
    public function filter_by_price_range(array $product_ids, array $config): array {
        ...
        return array_filter( $product_ids, function( $product_id ) use ( $min_price, $max_price, $price_type ) {
    ...
    public function filter_by_stock(array $product_ids, array $config): array {
        ...
        return array_filter( $product_ids, function( $product_id ) use ( $stock_status, $min_quantity, $max_quantity ) {
            $product = wc_get_product($product_id);
            if (!$product) {
                return false;
            }
    ...
    public function filter_by_attributes(array $product_ids, array $config): array {
        ...
        return array_filter($product_ids, function($product_id) use ($attributes, $operator) {
            $product = wc_get_product($product_id);
            if (!$product) {
                return false;
            }
    ...
    public function filter_by_tags(array $product_ids, array $config): array {
        ...
        if ( count( $tags ) > 100 ) {
            $this->logger->warning( 'Too many tags provided, limiting to 100', array(
                'count' => count( $tags )
            ) );
            $tags = array_slice( $tags, 0, 100 );
        }

        return array_filter($product_ids, function($product_id) use ($tags, $operator) {
    ...
    public function filter_by_rating(array $product_ids, array $config): array {
        ...
        return array_filter($product_ids, function($product_id) use ($min_rating, $max_rating, $min_reviews) {
            $product = wc_get_product($product_id);
            if (!$product) {
                return false;
            }
    ...
    public function filter_by_sales_performance(array $product_ids, array $config): array {
        ...
        return array_filter($product_ids, function($product_id) use ($start_date, $end_date, $min_sales, $max_sales) {
    ...
    public function filter_by_date_created(array $product_ids, array $config): array {
        ...
        return array_filter($product_ids, function($product_id) use ($start_date, $end_date) {
    ...
    public function apply_custom_filter(array $product_ids, array $config): array {
        ...
        try {
            return array_filter($product_ids, function($product_id) use ($callback, $args) {
                return call_user_func($callback, $product_id, $args);
            });
        } catch (Exception $e) {
            $this->logger->error('Custom filter failed', array(
    ...
    private function get_product_sales_count(int $product_id, string $start_date, string $end_date): int {
        ...
        if ( ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $start_date ) || ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $end_date ) ) {
            $this->logger->error( 'Invalid date format in sales count query', array(
        ...
        if ( false === $start_timestamp || false === $end_timestamp ) {
            $this->logger->error( 'Invalid date values in sales count query', array(
                'start_date' => $start_date,
                'end_date' => $end_date
            ) );
            return 0;
    ...
    public function validate_filters(array $filters): bool {
        $valid_filter_types = array(
            'category', 'price_range', 'stock', 'attributes', 
            'tags', 'rating', 'sales_performance', 'date_created', 'custom'
        );
