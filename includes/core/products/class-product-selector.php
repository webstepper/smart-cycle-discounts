...
class SCD_Product_Selector {
    ...
    public function select_products(array $criteria): array {
        ...
        try {
            ...
            $this->logger->info('Products selected successfully', array(
                'criteria' => $criteria,
                'count' => count($product_ids)
            ));

        } catch (Exception $e) {
            $this->logger->error('Product selection failed', array(
                'criteria' => $criteria,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function get_eligible_products(array $exclude_ids = array()): array {
        ...
        try {
            ...
            $this->logger->error('Failed to get eligible products', array(
                'error' => $e->getMessage(), 'exclude_ids' => $exclude_ids
            ));
        }
    ...
    public function filter_by_criteria(array $product_ids, array $criteria): array {
        ...
        $this->logger->debug('Products filtered by criteria', array(
    ...
    private function apply_post_query_filters(array $product_ids, array $criteria): array {
        ...
        if (!empty($criteria['product_type'])) {
            $filtered_ids = array_filter($filtered_ids, function($product_id) use ($criteria) {
                $product = wc_get_product($product_id);
    ...
    private function filter_by_price_range(array $product_ids, array $criteria): array {
        ...
        return array_filter($product_ids, function($product_id) use ($min_price, $max_price) {
            $product = wc_get_product($product_id);
            if (!$product) {
                return false;
            }
    ...
    private function filter_by_categories(array $product_ids, array $category_ids): array {
        return array_filter($product_ids, function($product_id) use ($category_ids) {
    ...
    private function filter_by_tags(array $product_ids, array $tag_ids): array {
        return array_filter($product_ids, function($product_id) use ($tag_ids) {
    ...
    private function filter_by_stock_status(array $product_ids, string $stock_status): array {
        return array_filter($product_ids, function($product_id) use ($stock_status) {
            $product = wc_get_product($product_id);
    ...
    private function filter_by_product_type(array $product_ids, string $product_type): array {
        return array_filter($product_ids, function($product_id) use ($product_type) {
            $product = wc_get_product($product_id);
    ...
    private function filter_by_featured_status(array $product_ids, bool $featured): array {
        return array_filter($product_ids, function($product_id) use ($featured) {
            $product = wc_get_product($product_id);
    ...
    private function filter_by_sale_status(array $product_ids, bool $on_sale): array {
        return array_filter($product_ids, function($product_id) use ($on_sale) {
            $product = wc_get_product($product_id);
    ...
    private function filter_by_attributes(array $product_ids, array $attributes): array {
        return array_filter($product_ids, function($product_id) use ($attributes) {
            $product = wc_get_product($product_id);
            if (!$product) {
                return false;
            }
    ...
    public function get_random_products(array $product_ids, int $count): array {
        // Validate count
        if ( $count <= 0 ) {
            $this->logger->warning( 'Random product count must be positive', array(
                'count' => $count
            ) );
            return array();
        }
    ...
    public function validate_and_filter_product_ids( array $product_ids ): array {
        ...
        foreach ( $product_ids as $product_id ) {
            ...
            if ( ! is_numeric( $product_id ) ) {
                $this->logger->warning( 'Invalid product ID type (not numeric)', array(
            ...
            if ( $product_id <= 0 ) {
                $this->logger->warning( 'Invalid product ID (zero or negative)', array(
                    'product_id' => $product_id
                ) );
                continue;
            }
            ...
            if ( false === $post_status ) {
                ...
                $this->logger->info( 'Product ID does not exist (may have been deleted)', array(
                    'product_id' => $product_id
                ) );
                continue;
            }
            ...
            if ( 'publish' !== $post_status ) {
                ...
                $this->logger->info( 'Product exists but not published', array(
                    'product_id' => $product_id,
                    'status' => $post_status
                ) );
                continue;
            ...
            if ( 'product' !== $post_type ) {
                $this->logger->warning( 'ID is not a product', array(
                    'product_id' => $product_id,
                    'post_type' => $post_type
                ) );
                continue;
    ...
    public function get_selection_statistics(): array {
        ...
        try {
            ...
            $in_stock_query = new WP_Query(array(
                'post_type' => 'product',
                'post_status' => 'publish',
            ...
            $this->logger->error('Failed to get selection statistics', array(
                'error' => $e->getMessage()
            ));
        }
    ...
    public function select_by_categories(array $category_ids, array $conditions = array(), int $limit = 0): array {
        ...
        try {
            ...
            $this->logger->info('Products selected by categories', array(
            ...
            $this->logger->error('Category-based product selection failed', array(
                'categories' => $category_ids,
                'error' => $e->getMessage()
            ));
            return array();
    ...
    public function get_product_count_by_categories(array $category_ids, array $conditions = array()): int {
        ...
        try {
            ...
            $this->logger->error('Product count by categories failed', array(
                'categories' => $category_ids,
                'error' => $e->getMessage()
            ));
            return 0;
    ...
    public function search_products_autocomplete(string $search_term, array $category_ids = array(), int $limit = 20): array {
        ...
        try {
            ...
            $this->logger->error('Product search failed', array(
    ...
    public function get_products_by_ids(array $product_ids): array {
        ...
        try {
            ...
            $this->logger->error('Get products by IDs failed', array(
    ...
    public function get_product_categories_hierarchy(bool $include_empty = false): array {
        ...
        try {
            $categories = get_terms(array(
                'taxonomy' => 'product_cat', 'hide_empty' => !$include_empty,
                'orderby' => 'name',
                'order' => 'ASC'
            ));

            if (is_wp_error($categories) || !is_array($categories)) {
                $this->logger->warning('Failed to get product categories', array(
            ...
            $this->logger->error('Get categories hierarchy failed', array(
                'error' => $e->getMessage(), 'include_empty' => $include_empty
            ));
            return array();
        }
    }
    ...
    public function process_selection_config(array $config): array {
        ...
        try {
            ...
            switch ($config['product_selection_type']) {
                ...
                case 'specific_products':
                    ...
                    if ( count( $product_ids ) < count( $raw_product_ids ) ) {
                        ...
                        $this->logger->warning( 'Some product IDs filtered out (deleted or invalid)', array(
            ...
            $this->logger->error('Product selection processing failed', array(
                'config' => $config,
                'error' => $e->getMessage()
            ));
    ...
    private function transform_conditions_for_engine(array $ui_conditions): array {
        ...
        foreach ( $ui_conditions as $condition ) {
            ...
            if ( ! isset( $condition['type'], $condition['operator'] ) ) {
                $this->logger->warning( 'Skipping invalid condition - missing type or operator', array(
                    'condition' => $condition
                ) );
                continue;
            }
            ...
            $this->logger->debug( 'Transformed condition', array(
                'from' => $condition,
                'to' => $engine_condition
            ) );
        }
    ...
    public function handle_product_deletion( int $product_id ): void {
        ...
        $this->logger->info( 'Product being deleted, invalidating caches', array(
            'product_id' => $product_id
        ) );
    ...
    public function handle_product_update( int $product_id ): void {
        ...
        $this->logger->debug( 'Product updated, invalidating caches', array(
            'product_id' => $product_id
        ) );
