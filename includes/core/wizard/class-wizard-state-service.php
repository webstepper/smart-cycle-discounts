...
class SCD_Wizard_State_Service {
	 ...
	 * @var      array
	 */
	private $steps = array( 'basic', 'products', 'discounts', 'schedule', 'review' );
	...
	public function save(): bool {
		...
		if ( ! $lock_acquired ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( '[State Service] Lock not acquired, skipping save' );
			}
			return false;
		}
		...
		try {
			...
			// Debug: Log complete session data before persisting
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( '[State Service] Saving complete session data:' );
	 ...
	 * Save step data.
	 *
	 * Merges new data with existing step data to preserve optional fields
	 * that were previously saved but not included in the current save.
	 *
	 * @since    1.0.0
	...
	public function save_step_data( string $step, array $data ): bool {
		...
		// Merge new data with existing data (new values override existing)
		$merged_data = array_merge( $existing_data, $data );

		$this->prepare_step_data( $step, $merged_data );
	...
	public function get_step_data( string $step ): array {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( '[State Service] Getting step data for "' . $step . '"' );
			error_log( '[State Service] All steps in session: ' . print_r( array_keys( isset( $this->data['steps'] ) ? $this->data['steps'] : array() ), true ) );
		}

		if ( ! isset( $this->data['steps'][ $step ] ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( '[State Service] Step "' . $step . '" not found in session, returning empty array' );
			}
			return array();
		}

		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( '[State Service] Returning data for "' . $step . '": ' . print_r( $this->data['steps'][ $step ], true ) );
		}
	 ...
	 *
	 * @since    1.0.0
	 * @param    bool    $is_draft    Draft status.
	 * @return   bool                 Success status.
	 */
	public function set_draft_status( bool $is_draft ): bool {
