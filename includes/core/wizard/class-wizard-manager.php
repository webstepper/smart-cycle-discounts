...
class SCD_Wizard_Manager {
    ...
    public function __construct(
        ...
        if ( function_exists( 'scd_debug_wizard' ) ) {
            scd_debug_wizard( 'constructor', array(
    ...
    public function initialize(array $options = array()): string {
        ...
        if ( function_exists( 'scd_debug_wizard' ) ) {
            scd_debug_wizard( 'initialize', 'init', array(
                'options' => $options,
        ...
        if ($this->state_service->has_session()) {
            try {
                ...
                SCD_Log::warning('Failed to clear existing wizard session during initialization', array(
                    'error' => $e->getMessage(),
                ...
                if ( function_exists( 'scd_debug_error' ) ) {
                    scd_debug_error( 'Failed to clear wizard session', $e, array(
                        'action' => 'initialize',
                    ) );
                }
        ...
        if ( function_exists( 'scd_debug_wizard' ) ) {
            scd_debug_wizard( 'session_created', array(
        ...
        try {
            ...
            SCD_Log::error('Failed to save initial wizard metadata', array(
                'error' => $e->getMessage(),
            ...
            if ( function_exists( 'scd_debug_error' ) ) {
                scd_debug_error( 'Failed to save wizard metadata', $e, array(
                    'action' => 'initialize',
                    'session_id' => $session_id,
                ) );
            }
        ...
        if ( function_exists( 'scd_debug_wizard' ) ) {
            scd_debug_wizard( 'initialize', 'complete', array(
                'session_id' => $session_id,
                'initial_data' => $initial_data,
    ...
    public function save_step_data(string $step, array $data): void {
        try {
            ...
            SCD_Log::error('Failed to save wizard step data', array(
                'step' => $step,
                'error' => $e->getMessage(),
    ...
    public function complete_wizard(): array {
        ...
        $has_basic = !empty($all_data['basic']['name']);
        $has_products = !empty($all_data['products']['product_selection_type']);
        $has_discounts = !empty($all_data['discounts']['discount_type']);
        
        if (!$has_basic || !$has_products || !$has_discounts) {
            $missing = array();
            if (!$has_basic) $missing[] = 'basic information';
            if (!$has_products) $missing[] = 'product selection';
            if (!$has_discounts) $missing[] = 'discount configuration';
            
            if (defined('WP_DEBUG') && WP_DEBUG) {

            }
            
            return $this->error_response('Please complete all required steps: ' . implode(', ', $missing));
        }

        // Compile campaign data using compiler
        $compiler = $this->get_compiler();
        $all_steps_data = $this->get_all_campaign_data();
        ...
        try {
            ...
            if (!$saved_campaign) {
                return $this->error_response('Failed to save campaign');
            }
        } catch (Exception $e) {
            // Log error

            return $this->error_response('Failed to save campaign: ' . $e->getMessage());
        }
        ...
        try {
            ...
            SCD_Log::warning('Failed to clear wizard session after completion', array(
                'error' => $e->getMessage(),
                'campaign_data' => is_object($saved_campaign) ? get_class($saved_campaign) : 'unknown'
            ));
        ...
        if (is_object($saved_campaign)) {
            if (method_exists($saved_campaign, 'get_id')) {
                $campaign_id = $saved_campaign->get_id();
            } elseif (property_exists($saved_campaign, 'id')) {
        ...
        if (is_object($saved_campaign)) {
            if (method_exists($saved_campaign, 'get_uuid')) {
                $campaign_uuid = $saved_campaign->get_uuid();
            } elseif (property_exists($saved_campaign, 'uuid')) {
                $campaign_uuid = $saved_campaign->uuid;
            }
        }
    ...
    private function mark_step_completed(string $step): void {
        try {
            ...
            SCD_Log::warning('Failed to mark wizard step as completed', array(
                'step' => $step,
                'error' => $e->getMessage()
            ));
