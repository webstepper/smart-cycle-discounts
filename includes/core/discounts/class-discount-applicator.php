...
class SCD_Discount_Applicator {
    ...
    public function apply_to_product(int $product_id, array $discount_config, array $context = array()): array {
        ...
        try {
            ...
            // Calculate discount
            $calculation_context = array_merge($context, array(
                'product_id' => $product_id,
                'product' => $product,
            ...
            if ( $calculation['valid'] && 0 < $calculation['discount_amount'] ) {
                ...
                $this->logger->info('Discount applied to product', array(
                    'product_id' => $product_id, 'original_price' => $original_price,
            ...
            $this->logger->error('Failed to apply discount to product', array(
                'product_id' => $product_id,
                'error' => $e->getMessage()
            ));
        }
        ...
        foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
            try {
                ...
                $results['errors'][] = sprintf(
                    __('Error processing cart item %s: %s', 'smart-cycle-discounts'),
                    $cart_item_key,
                    $e->getMessage()
                );
            }
    ...
    public function apply_to_cart_item(string $cart_item_key, array $cart_item, array $discount_rule): array {
        ...
        try {
            ...
            $result['errors'][] = $e->getMessage();
            $this->logger->error('Failed to apply discount to cart item', array(
                'cart_item_key' => $cart_item_key,
                'error' => $e->getMessage()
            ));
        }
    ...
    public function update_display_price(WC_Product $product, array $discount_config): bool {
        ...
        if ( ! current_user_can( 'edit_products' ) && ! did_action( 'woocommerce_before_calculate_totals' ) ) {
            $this->logger->warning('Unauthorized attempt to update display price', array(
                'product_id' => $product->get_id(),
        ...
        try {
            ...
            $calculation = $this->discount_engine->calculate_discount(
                $original_price,
                $discount_config,
                array('product_id' => $product_id, 'product' => $product)
            );
            ...
            if ( $calculation['valid'] && 0 < $calculation['discount_amount'] ) {
                ...
                $this->logger->debug('Display price updated', array(
                    'product_id' => $product_id, 'original_price' => $original_price,
                    'discounted_price' => $calculation['discounted_price']
                ));
            ...
            $this->logger->error('Failed to update display price', array(
                'product_id' => $product->get_id(),
                'error' => $e->getMessage()
            ));
        }
    ...
    public function is_product_eligible(WC_Product $product, array $context = array()): bool {
        ...
        $allowed_types = apply_filters('scd_allowed_product_types', array(
            'simple', 'variable', 'variation', 'grouped'
        ));
    ...
    public function bulk_apply_discounts(array $product_ids, array $discount_config, array $context = array()): array {
        ...
        $this->logger->info('Bulk discount application completed', array(
            'total_products' => $results['total_products'], 'successful' => $results['successful_applications'],
