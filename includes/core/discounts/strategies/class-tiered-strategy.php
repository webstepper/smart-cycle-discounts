...
class SCD_Tiered_Strategy implements SCD_Discount_Strategy_Interface {
        ...
        if ( ! isset( $tier['discount_value'] ) || ! is_numeric( $tier['discount_value'] ) ) {
            ...
            if ( 'percentage' === $discount_type ) {
                ...
                if ( 1000000 < $discount_value ) {
                    ...
                    $errors[] = sprintf(
                        __( '%s: Fixed discount of %s per item seems very high. Consider using "Order Total" mode or percentage instead.', 'smart-cycle-discounts' ),
                        $tier_label,
    ...
    public function get_tier_description( array $tier, string $apply_to = 'per_item' ): string {
        ...
        if ( 'order_total' === $apply_to ) {
            ...
            if ( 'percentage' === $discount_type ) {
                return sprintf(
                    __( '%s%% off your order when purchasing %s or more items', 'smart-cycle-discounts' ),
            ...
            if ( 'percentage' === $discount_type ) {
                return sprintf(
                    __( '%s%% off each item when purchasing %s or more', 'smart-cycle-discounts' ),
                    number_format( $discount_value, 1 ),
                    number_format( $min_quantity )
                );
            } else {
